...
// ObjectStore creates object given an allocator.
// It provides APIs to create/get/seal/delete objects.
class ObjectStore {
 public:
  explicit ObjectStore(IAllocator &allocator);

  /// Create a new object given object's info. Abort if
  /// an object with the same id exists.
  ///
  ...
  /// \return
  ///   - pointer to created object when succeeds, nullptr when out of space.
